defmodule MapleTreeWeb.Live.Components.Navbar do
  ...
  @impl true
  def handle_event("click_dark_mode", %{"theme_checkbox_mobile" => %{"toggle" => checkbox_value}}, %{assigns: %{current_user: %{id: user_id}}} = socket) do
    if connected?(socket), do: update_checkbox_theme(checkbox_value, user_id)
    {:noreply, socket}
  end

  @impl true
  def handle_event("click_dark_mode", %{"theme_checkbox" => %{"toggle" => checkbox_value}}, %{assigns: %{current_user: %{id: user_id}}} = socket) do
  ...
  defp update_checkbox_theme(checkbox_value, user_id) do
    theme = if Converters.string_to_boolean(checkbox_value) == true do "dark" else "light" end
